...
namespace TheWarofThreads
{
    class Program
    {
        ...
        //вставки WINAPI запрещают изменять размер консоли и растягивать 
        ...
        static void Main(string[] args)
        {
            ...
            while (true){                                                     //цикл управление пушкой
                writeat(x, y, '|');                                           //отрисовка пушки
                switch (Console.ReadKey(true).Key){                           //считывает нажатые клавиши без записи в консоль
                    ...
                    case ConsoleKey.LeftArrow:                                //нажата левая стрелка
                        if (x - 1 >= 2){                                      //если есть место для движения влево
                            startevt.Set();
        ...
        static void bullet()//выстрел
        {
            ...
            try { bulletsem.Release(); }                                      //если меньше 3х-ок, больше завершаем поток
            catch { return; }
            while (y != -1){                                                  //пока в консоли
        ...
        static void badguy()//враг
        {
            ...
            while ((dir == 1 && x != conMaxWidth - 2 && !hitme) || (dir == -1 && x != 2 && !hitme)){
                writeat(x, y, badchar[x % 4]);                                //отрисовка врагов
                for (int i = 0; i < 10; i++){
                    Thread.Sleep(5 / ((int)hit + (int)miss + 1)+10);         //ускорение
                    if (getat(x, y) == '*'){                                  //если "пуля" попала
                ...
                if (hitme) {Interlocked.Increment(ref hit); score(); return;} //если во врага попали то изменяем счет и завершаем поток
                else x += dir;                                                //иначе изменяем координаты врага
            }
            if (!hitme) { Interlocked.Increment(ref miss); score(); }         //если во врага так и не попали то изменяем счет
        }
        ...
        {
            if (miss > 29){                                                   //если количество промахов больше или равно 30
                lock (gameover){
